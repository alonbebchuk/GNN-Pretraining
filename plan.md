...
*   **2.2. Datasets and Experimental Setups**
    *   **2.2.1. Datasets**
        ...
        *   **Data Splits:**
        *   **Pre-training:** To ensure reproducibility, each source dataset is individually split into 90% training and 10% validation sets using a fixed random seed. These splits are saved and reused. Training proceeds in balanced, synchronous steps with per-domain PyG batching: at each step, one mini-batch per domain is drawn using with-replacement sampling. An epoch length equals the maximum number of steps among domains, ensuring a complete pass through all datasets. Validation sets from each domain are kept separate to support a domain-balanced metric.
            *   **Implementation Details:** Data loading uses `BalancedMultiDomainSampler` which allocates 32 total graphs per step across domains (32÷num_domains per domain). Each step yields a `Dict[str, Batch]` where keys are domain names and values are PyG batches containing the sampled graphs for that domain.
            *   **Downstream Tasks:** For the TUDatasets, a fixed 80% training, 10% validation, and 10% test stratified split is generated once with a fixed random seed and reused across all runs. `Cora` and `CiteSeer` use their standard public splits, which are inherently fixed.
                *   **Implementation Details:** Single-domain data loaders use standard PyG `DataLoader` with batch size 32. All loaders use reproducible random generators and worker seeding for deterministic behavior across runs.
    *   **2.2.2. Pre-training Setup**
        ...
        *   **Pre-training Tasks and Losses:**
            ...
            *   **ii. Self-Supervised (Contrastive)**
                *   **Node-level (SimCLR-style):** This task contrasts augmented views of nodes using GraphCL methodology with independent node dropping. For each graph, two augmented versions (`G'` and `G''`) are created using **independent augmentation**: node dropping is always applied, edge dropping and feature masking are applied independently with probabilities `p=0.2` each. This follows GraphCL's proven strategy for biochemical molecule datasets, ensuring diverse augmented views while maintaining graph semantic meaning. Since independent node dropping creates different node sets per view, only nodes present in both views can form valid positive pairs. A node intersection function identifies common nodes between views, and only embeddings from these intersecting nodes participate in contrastive learning, ensuring correct positive pair alignment.
                    *   **GraphCL Augmentation Details:** Following GraphCL defaults: node dropping removes 20% of nodes, edge dropping removes 20% of edges, and feature masking zeros out 20% of feature dimensions. Minimum thresholds ensure stability: augmentations are only applied if graphs have at least 3 nodes, 3 edges, and 3 features respectively.
                    *   **Intersection-Based Contrastive Learning:** After independent augmentation, each view may have different surviving nodes. The intersection of nodes present in both views is computed, and only embeddings from these common nodes are used for contrastive learning. This ensures that `z1[i]` and `z2[i]` correspond to the same original node, making positive pairs meaningful.
                    *   **Projection Head:** Domain-specific `MLPHead(dim_in=256, dim_hidden=256, dim_out=128)`. Loss: NT-Xent using cosine similarity and fixed temperature `τ=0.1`. Implementation uses the standard SimCLR approach where embeddings from intersecting nodes in both views are concatenated and a similarity matrix is computed for the NT-Xent loss.
                *   **Graph-Level (InfoGraph-style):** This task maximizes the mutual information between a graph's summary vector `s` and its node embeddings `h_v`. The summary vector `s` is computed by taking the mean of the final-layer node embeddings for all nodes in the graph.
                    ...
                    *   **Negative Pairs & Ratio:** For the same graph summary `s`, negative pairs are `(s, h_u)` where `u` is a node from any *other* graph in the same batch. This means all nodes from other graphs in the batch serve as negatives, making explicit negative sampling unnecessary. The positive-to-negative ratio for a given graph is therefore `N` to `(M - N)`, where `N` is the number of nodes in the graph and `M` is the total number of nodes in the batch.
                     *   **Implementation Details:** For each graph in the batch, positive scores are computed between the graph summary and all nodes from the same graph. Negative scores are computed between the graph summary and all nodes from other graphs in the batch. All positive and negative scores are collected and treated as a binary classification problem.
            ...
            *   **iv. Domain-Adversarial Objective (to promote transferability)**
                ...
                *   **Implementation Details:** Graph embeddings are computed via global mean pooling of node embeddings. Each domain is assigned a unique integer ID for classification. All graph embeddings from all domains in a batch are concatenated and processed jointly by the domain classifier.
        ...
        *   **Combined Pre-training Loss:**
            *   The total loss function is optimized from the perspective of the GNN backbone. Tasks are balanced using uncertainty weighting following Kendall et al., which adaptively balances tasks by learning task-dependent uncertainty parameters, and the domain-adversarial objective is incorporated via a negatively-weighted term:
            ...
            *   **Uncertainty Weighting Implementation:** Each task (except domain-adversarial) has a learnable log-variance parameter `log(σ²)`. The weighted loss for task `i` is computed as: `0.5 * exp(-log(σ²)) * L_i + 0.5 * log(σ²)`, which simplifies to the Kendall formula above. Tasks are additionally scaled by fixed coefficients: node feature masking (0.1), graph properties (0.3), node contrastive (1.0), graph contrastive (1.5), link prediction (1.5), and domain adversarial (1.0).
            *   **Domain-Adversarial Integration:** The negative sign on the domain loss term (`L_domain`) is crucial. By minimizing `L_total`, the GNN backbone is implicitly trained to *maximize* the domain classifier's loss, forcing it to produce domain-invariant embeddings. The domain classifier itself is trained to *minimize* `L_domain` through the normal forward pass, while the GRL ensures the backbone receives the adversarial signal. The adversarial weight `λ` follows the DANN schedule: `λ_p = (2 / (1 + exp(-γ * p))) - 1`, where `p` is the training progress from 0 to 1 and `γ = 10`.
...
*   **Training Details:** Training uses epochs × steps_per_epoch, where steps_per_epoch is the maximum number of batches among domains (ensuring complete coverage of all datasets per epoch). Total batch size is 32, allocated per domain (32÷num_domains). Batches are formed using the balanced, synchronous strategy in Sec. 2.2.1 with per-domain PyG batching. We use cosine annealing with a 10% linear warm-up. GRL lambda follows the DANN schedule. Multi-task losses are balanced via per-task uncertainty weighting shared across domains.
