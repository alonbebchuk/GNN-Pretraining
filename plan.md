    ...
    *   **2.2.2. Pre-training Setup**
        ...
        *   **Pre-training Tasks and Losses:**
            ...
            *   **ii. Self-Supervised (Contrastive)**
                *   **Node-level (SimCLR-style):** This task contrasts augmented views of nodes using the canonical SimCLR approach with in-view + cross-view negatives. For each graph, two augmented versions (`G'` and `G''`) are created using an optimized augmentation strategy: **shared node dropping** is applied to both views (ensuring identical node sets), followed by **independent edge dropping** and **independent feature masking** for each view (each with `p=0.5` probability). This approach ensures proper positive pair alignment while learning invariance to edge connectivity and feature perturbations. A positive pair consists of the same node viewed through these two different augmentations (`h_v'` and `h_v''`). For a given node's embedding `h_v'`, negative pairs are formed with **all other embeddings in both views** (2N-2 negatives for batch size N), maximizing the negative sampling for richer training signal. The embeddings from both views are concatenated and a single large similarity matrix is computed for the NT-Xent loss.
                    *   **Projection Head:** Domain-specific `MLPHead(dim_in=256, dim_hidden=256, dim_out=128)`. Loss: NT-Xent (Normalized Temperature-scaled Cross-Entropy) using cosine similarity and a fixed temperature `τ=0.1`. Implementation uses the canonical SimCLR approach where embeddings from both views are concatenated (`z = [z1; z2]` shape `[2N, D]`) and the similarity matrix is computed as `(z @ z.T) / τ` with shape `[2N, 2N]`. The loss treats `(i, i+N)` as positive pairs while ignoring diagonal self-similarities.
                    *   **Implementation Optimization:** The shared node dropping strategy eliminates the need for complex intersection computations between augmented views, resulting in more efficient training and stronger learning signals by ensuring all nodes contribute to the contrastive loss.
