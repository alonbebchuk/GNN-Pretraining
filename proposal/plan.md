    ...
    *   **2.2.2. Pre-training Setup**
        ...
        *   **Pre-training Tasks and Losses:**
            ...
            *   **iii. Supervised Auxiliary Task**
                *   **Graph Property Prediction:** Regress on key structural properties: number of nodes, number of edges, and the average clustering coefficient. To prevent scale dominance, all target properties are z-score standardized across the training set before computing the loss.
                    *   **Head:** `MLPHead(dim_in=256, dim_hidden=256, dim_out=3)`. Loss: MSE.
            *   **iv. Domain-Adversarial Objective (to promote transferability)**
                *   To explicitly encourage the GNN to learn domain-invariant features, we use an adversarial setup. A dedicated classifier head is trained to predict a graph's source domain. A **Gradient Reversal Layer (GRL)** is placed between the GNN backbone and this classifier. During backpropagation, the GRL reverses the gradient flowing from the domain classifier to the GNN, training the backbone to produce embeddings that *confuse* the classifier, thus promoting domain-invariance.
...
*   **Splits:**
    ...
    *   Use a fixed global random seed of **42** for all splits to ensure reproducibility.
...
*   **Adversarial Training Loop (`src/trainer.py`):**
    *   The domain-adversarial setup requires a two-step optimization process within each training step. This typically requires two separate optimizers: one for the GNN backbone and all other heads/parameters, and one exclusively for the domain classifier head.
    1.  **Train the Domain Classifier:** Perform a forward pass and calculate the domain classification loss (`L_domain`). Backpropagate this loss *only* through the domain classifier head and update its weights. Do not update the GNN backbone in this step.
    2.  **Train the GNN Backbone:** Perform a forward pass to calculate all main task losses and the domain loss again. Use the GRL to reverse the gradient from the domain loss. Backpropagate the combined loss (`L_total`) and update the weights of the GNN backbone and the uncertainty parameters. The domain classifier's weights should be frozen during this step.
